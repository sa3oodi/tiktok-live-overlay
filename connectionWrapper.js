...
class TikTokConnectionWrapper extends EventEmitter {
    ...
    connect(isReconnect) {
        this.connection.connect().then((state) => {
            ...
            // Enhanced error handling for common TikTok connection issues
            let errorMessage = err.toString();
            
            if (errorMessage.includes('status')) {
                errorMessage = 'Failed to connect to TikTok servers. The user might not be live or the username is incorrect.';
            } else if (errorMessage.includes('LIVE has ended')) {
                errorMessage = 'The live stream has ended.';
                this.reconnectEnabled = false;
